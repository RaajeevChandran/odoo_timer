...
class CreateTimerBloc extends Bloc<CreateTimerEvent, CreateTimerState> {
  CreateTimerBloc() : super(CreateTimerInitial(tasks: dummyTasks)) {
    ...
    on<CreateTimerButtonTapEvent>(_onCreateTimerButtonTap);
  }

  void _onFormValueChanged(
      FormValueChanged event, Emitter<CreateTimerState> emit) {
  ...
  void _onCreateTimerButtonTap(
      CreateTimerButtonTapEvent event, Emitter<CreateTimerState> emit) {
    if (state is CreateTimerInitial) {
      ...
      if (currentState.project != null && currentState.task != null) {
        ...
        emit(CreateTimerFormValidationSuccess(
